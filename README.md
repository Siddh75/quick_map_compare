...
Release S to go back to the normal canvas view. If the armed tile's own content is still loading (its "Loading…" badge is showing) — including a reload triggered by panning or zooming the main canvas, not just the tile's first load — the cursor over the main canvas switches to a busy/spinner shape, a sign that pressing S right now won't have much to show yet.
